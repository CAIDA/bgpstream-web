...
class LocalBgpArchive implements BgpArchiveInterface {

    const ARCHIVE_PATH = "http://bgp-archive.example.com";

    private $allowedRanges = [
        'a.b.c.d/24', // same range to allow access for
    ];
    ...
    {
        // allow local users to pull data directly from other archives
        $preferRemote = $request->get('preferRemote', 'true');
        $preferRemote = $preferRemote == 'true' ? true : false;
        if(!$preferRemote) {
            return false;
        }
